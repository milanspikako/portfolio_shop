...
if (isset($formData["submit"]) && $formData["submit"] == "Save") {
  ...
  if (empty($formErrors)) {
    ...
    if (!move_uploaded_file($imageFileTmpPath, $destinationPath)) {
      ...
      $insertStatus = createProduct($formData, $connection);
      if ($insertStatus) {
        $systemMessage = "Product created successfully";
      } else {
        $systemMessage = "Something was wrong with insert product";
      }
    }
  }
}
...
include_once '../../../views/products/createView.php';
